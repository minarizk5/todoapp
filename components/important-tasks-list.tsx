...
export function ImportantTasksList() {
  ...
  // Update tasks when context is available - filter by user ID and important flag
  useEffect(() => {
    if (user && user.id) {
      // Only show important tasks for the current user
  ...
  return (
    <div className="space-y-4">
      {tasks.length > 0 ? (
        ...
        <p className="text-center text-muted-foreground py-4">No important tasks yet.</p>
      )}
    </div>
  )
}
